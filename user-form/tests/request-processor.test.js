...
describe("request-processor", () => {
  ...
  describe("processRequest", () => {
    test("rejects request without an extension", () => {
      expect(() => underTest.processRequest("req-id")).toThrow(
        "no extension provided"
      );
    });
    test("rejects requests with an unsupported extension", () => {
      expect(() => underTest.processRequest("req-id", "xls")).toThrow(
        "extension xls is not supported"
      );
    });
    test("signs a request for supported extensions", () => {
      expect(underTest.processRequest("req-id", "jpg")).toEqual({
